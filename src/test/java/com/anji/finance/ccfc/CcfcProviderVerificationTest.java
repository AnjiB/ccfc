...
public class CcfcProviderVerificationTest {
	...
	private File specFile = new File("docs/oas_contract/ccfc_provider_contract.json");

	private final OpenApiValidationFilter validationFilter = new OpenApiValidationFilter(specFile.getAbsolutePath());

	@Test
	public void testFraudScoreForValidCreditCard() throws Exception {

		FraudTestClient ftc = new FraudTestClient(HOST + valueOf(port));
		ApiResponse response = ftc.getFraudScoreForValidCreditCard(VALID_CC, "2023-03-31",
				Map.of("Authorization", USER_CREDS), asList(validationFilter));
	...
	public void testErroMessageForInvalidCreditCard() throws Exception {
		...
		FraudTestClient ftc = new FraudTestClient(HOST + valueOf(port));
		ApiResponse response = ftc.getFraudScoreForValidCreditCard("test", "2023-03-31",
				Map.of("Authorization", USER_CREDS), asList(validationFilter));
	...
	/*
	 * @Test public void testErroMessageForInvalidExpDate() throws Exception {
	 * 
	 * FraudTestClient ftc = new FraudTestClient(HOST + valueOf(port)); ApiResponse
	 * response = ftc.getFraudScoreForValidCreditCard(VALID_CC, "2023-02-31",
	 * Map.of("Authorization", USER_CREDS), asList(validationFilter));
	 * assertThat(response.getStatusCode()).isEqualTo(400); var body =
	 * response.getResponseBodyAs(CCFraudInfo.class);
	...
	public void testErroMessageForInvalidCreds() throws Exception {
		...
		FraudTestClient ftc = new FraudTestClient(HOST + valueOf(port));
		ApiResponse response = ftc.getFraudScoreForValidCreditCard(VALID_CC, "2023-03-31",
				Map.of("Authorization", "Basic YW5qaWFkbWluZGQ6YWdvb2RxYXBlcnNvbg=="), asList(validationFilter));
